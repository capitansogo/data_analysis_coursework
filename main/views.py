    ...
    if request.method == 'POST':
        if 'file' in request.FILES:
            ...
            if file.name.endswith('.csv'):
                ...
                # Обучаем модель
                model.fit(X, y)
                yr_lin = "y = " + str(round(model.coef_[0], 5)) + "x + " + str(
                ...
                # Обучаем модель
                model.fit(X, y_log)
                yr_log = "y = e^(" + str(round(model.coef_[0], 5)) + "*x + " + str(
                    round(model.intercept_, 5)) + ")"
                print(yr_log)
                ...
                yr_inverse = "y = " + str(round(model.coef_[0],5)) + "/x + " + str(
                    round(model.intercept_, 5))
                print(yr_inverse)
                ...
                return render(request, 'success.html',
                              {'average_level': round(average_level, 2), 'growth': round(growth, 2),
                               ...
                               'yr_linear': yr_lin,
                               'yr_exponential': yr_log,
                               'yr_hyperbolic': yr_inverse,
                               })
